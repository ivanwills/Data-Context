...
our @EXPORT_OK   = qw/lol_path lol_itterate/;
our %EXPORT_TAGS = ();
#our @EXPORT      = qw//;
    ...
    if ( $point ) {
        if ( !ref $point ) {
            ...
            for my $key ( keys %$point ) {
                ...
                lol_itterate( $point->{$key}, $code, "$path$key" ) if ref $point->{$key};
            }
        }
        elsif ( ref $point eq 'ARRAY' ) {
            for my $i ( 0 .. @$point - 1 ) {
                $code->( $point->[$i], "$path$i" );
                lol_itterate( $point->[$i], $code, "$path$i" ) if ref $point->[$i];
            }
        }
    }
