...
any format that allows arbitary depth datastructures (eg not ini style files).
...
position at run time.This consists of a dot seperated list of hash keys or
...
path throught the config and a reference to the L<Data::Context::Instance>
...
they are encountered. This allows an action to inlucde the output of
...
method will be called before other non ordered methods (but in increasind
numberical order by C<ORDER>). If the value is negative then it will be
run after all non ordered actions (again -ve C<ORDER>s will be run in
increasing numberical order ie most negative to lease negative).
